...
def train(FLAGS):
    ...
    train_loader = cycle(DataLoader(train_image, batch_size=BATCH_SIZE, shuffle=True, num_workers=0, drop_last=True))

    val_image = VOC_Dataset("../data_csv/val.csv",INPUT_WIDTH,INPUT_HEIGHT)
    val_loader = cycle(DataLoader(val_image, batch_size=BATCH_SIZE, shuffle=True, num_workers=0, drop_last=True))
    ...
    Img = torch.FloatTensor(BATCH_SIZE, 3, INPUT_WIDTH, INPUT_HEIGHT).cuda()
    Label = torch.LongTensor(BATCH_SIZE, INPUT_WIDTH, INPUT_HEIGHT).cuda()
    ...
    for epoch in range(FLAGS.start_epoch, FLAGS.end_epoch):
        ...
        model.train()
        for iteration in range(int(len(train_image) / BATCH_SIZE)):
            ...
            writer.add_scalar('Cross Entropy Loss', loss.data.storage().tolist()[0],
                              epoch * (int(len(train_image) / BATCH_SIZE) + 1) + iteration) 


        if (epoch + 1) % 10 == 0 or (epoch + 1) == FLAGS.end_epoch: 
            model.eval()
            with torch.no_grad(): 
                for iteration in range(int(len(val_image) / BATCH_SIZE)):
